...
export default function SubcategoriesManager({ initialSubcategories, categories }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      let saved: Subcategory = await res.json()

      if (!isEdit && pendingPhoto) {
        const fd = new FormData()
        fd.append('file', pendingPhoto)
        const photoRes = await fetch(`/api/admin/subcategories/${saved.id}/photo`, { method: 'PUT', body: fd })
                ...
                <tbody className="divide-y divide-gray-100">
                  {subs.map((sub) => (
                    <tr key={sub.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-center text-gray-500 w-20 hidden sm:table-cell">{sub.sortOrder}</td>
